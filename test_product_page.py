...
class TestUserAddToBasketFromProductPage():
    ...
    @pytest.mark.parametrize('link_part', ["?promo=offer0", "?promo=offer1", "?promo=offer2",
                                           "?promo=offer3", "?promo=offer4", "?promo=offer5",
                                           "?promo=offer6",
                                           pytest.param("?promo=offer7", marks=pytest.mark.xfail),
                                           "?promo=offer8", "?promo=offer9"])
    def  test_user_can_add_product_to_basket(self, browser, link_part):
        link = f"http://selenium1py.pythonanywhere.com/catalogue/coders-at-work_207/{link_part}"
        # Открыть страницу
